...
- **Automated tests added (follow-up, user-requested):** installed Vitest and extracted the pure logic (`resolveCompanyName`, `extractCompanyFromText`, `revenueRangeToEstimate`, `isCacheFresh`) into `company-intelligence-utils.ts` so it's testable without a DB or OpenAI key — importing `company-intelligence.ts` directly would otherwise throw at module load (constructs an `OpenAI` client and imports `storage`→`db`, which throws if `DATABASE_URL`/`OPENAI_API_KEY` are unset, confirmed neither is set except `DATABASE_URL`). 17 tests written; **two real bugs were caught and fixed by the tests, not introduced for the demo:**
  1. The keyword-extraction regex used a single `/i` flag across the whole pattern, which silently defeated its own "must start with a capital letter" check (lowercase text could match). Rewrote as an explicit word-based scan instead of one combined regex.
  2. Separately, even after removing `/i`, the original single-regex approach backtracked its inner greedy quantifier before its repetition count, so "Initech Demo" could resolve to the technically-valid-but-wrong split "Initech Dem" + "o" before ever trying "Initech" + "Demo". This is exactly the kind of regex correctness bug that's easy to ship silently without a test — the story's Dev Notes originally called this heuristic "not NLP-grade parsing, revisit if unresolved-rate proves too high," and this is precisely the class of bug that assumption anticipated.
- No test framework existed in this repo before this session (see original note below, kept for history) — Vitest was added specifically for this story's testable logic per user request, not wired into a broader CI/test strategy.
- ~~No automated tests added: confirmed (again, at implementation time) that no test framework (vitest/jest/mocha) exists in this repo. Per this story's own Dev Notes, introducing one is out of scope.~~ *(Superseded — see above. User opted in to adding tests after story completion.)*
...
- 2026-07-08 — Added Vitest + 17 unit tests for the story's pure logic (company resolution, revenue parsing, cache freshness), extracting it into `company-intelligence-utils.ts` to make it testable without a DB/API key. Tests caught and led to fixing two real bugs in the company-name-extraction regex.
- 2026-07-08 — Code review (fresh context) found 2 bugs in this story's scope, both fixed:
  1. **Cache-key casing mismatch (AC #3/#4):** `companyCache.companyName` is documented as "normalized: lowercased, trimmed" (schema.ts), but neither `domainToCompanyName` nor `extractCompanyFromText` actually lowercase their output, and the title/description heuristic can return different casing for the same real company across different meetings (e.g. "ACME Corp" vs "Acme Corp"). That mismatch meant repeat lookups could miss the existing cache row, causing duplicate research calls and inconsistent per-meeting data instead of one shared record. Fixed with a surgical change in `server/storage.ts`: `getCompanyCache`/`getCompanyCacheByName` now match case-insensitively (`lower(companyName) = lower(:input)`), so casing drift in the extraction heuristic can no longer produce duplicate cache rows via `upsertCompanyCache`. Left the stored/displayed casing (and the schema comment) as-is — this fixes the functional dedup bug without changing what reps see on the meeting card.
  2. **Sequential enrichment serializes calendar scans (NFR-2 risk):** `google-calendar-integration.ts` and `outlook-integration.ts` awaited `companyIntelligenceService.enrich()` (up to a 30s timeout) one meeting at a time inside the import loop — a scan with several newly-imported, uncached companies could take minutes. Restructured both `scanAndImportMeetings()` methods to a two-phase flow: sequential dedupe-check + `createMeeting` (fast, DB-bound) first, then a bounded-concurrency (5 at a time) pass over the newly-created meetings for enrich→qualify. Per-meeting enrich-before-qualify ordering (AD-8, AD-11) is unchanged; only meetings within the same batch now research in parallel instead of one-by-one.
  - `npm run check` (0 errors) and `npm run test` (17/17 passing) both re-verified after these fixes.
  - **5 other findings from the same review are out of this story's scope** (different files/features, not touched by Story 1.1's task list) and are left for separate follow-up: a contradictory `status` filter in `calendar-cleanup.ts` that makes calendar auto-cleanup never match any row; `no-show-reschedule.ts` building a personalized reschedule email but sending a generic hardcoded follow-up instead; `qualified-email-service.ts` using `meeting.description` for the `{{meeting_link}}` placeholder; the Calendly webhook's `userId` fallback still violating the `meetings.userId` FK constraint; and a duplicated `MorningBriefingService` class across `morning-briefing.ts`/`pre-meeting-summary.ts`.
- 2026-07-08 — Follow-up code review (run after Story 1.2 landed on top of this story's pipeline) found 2 more bugs in this story's code, both fixed:
  1. **Race condition in `upsertCompanyCache` newly exposed by the bounded-concurrency fix above:** the select-then-insert-or-update in `server/storage.ts` was never atomic, but was previously safe because meetings were enriched strictly one at a time. Once enrichment runs 5-at-a-time concurrently, two meetings in the same scan batch that resolve to the same never-before-seen company (e.g. two attendees from the same domain — common for this app's actual use case of multiple stakeholders at one prospect) can both miss the cache, both call OpenAI, and race on the insert; the loser hit the unique-index violation, which was caught by `enrich()`'s generic error handler and mislabeled as a research **timeout**, silently skipping qualification for that meeting. Fixed by catching the insert failure in `upsertCompanyCache` and re-checking for a just-inserted row before giving up — converges to a single row via update instead of surfacing a spurious error.
  2. **`extractCompanyFromText` regression on hyphen/colon-separated titles:** the word-based rewrite (done earlier to fix a regex-backtracking bug, see the 2026-07-08 Vitest entry above) required every candidate word to start with an uppercase letter, which a lone `"-"` token never does — so `"Acme Corp - Demo"` stopped resolving at all (previously resolved to `"Acme Corp"`). A colon directly attached to a word (`"Initech: Demo"`) resolved but leaked the trailing colon into the extracted name (`"Initech:"`), which would fail to match a domain-resolved `"Initech"` cache entry for the same company. Fixed in `company-intelligence-utils.ts` by dropping a standalone separator token and stripping trailing punctuation from the last candidate word before the capitalization check. Added 2 regression tests.
  - `npm run check` (0 errors) and `npm run test` (19/19 passing) re-verified after these fixes.
...
- `server/services/company-intelligence-utils.ts` (new) — pure, dependency-free extraction of `resolveCompanyName`, `extractCompanyFromText`, `revenueRangeToEstimate`, `isCacheFresh` from `company-intelligence.ts`, so they're unit-testable without a DB connection or OpenAI API key
- `server/services/company-intelligence.test.ts` (new) — 17 unit tests covering company resolution, the keyword-extraction heuristic, revenue-range parsing, and the 24h cache-freshness boundary
- `vitest.config.ts` (new) — root-level test config; kept separate from `vite.config.ts` because that file pins `root: client/` for the production build, which would otherwise scope test discovery to `client/` only
- `package.json` / `package-lock.json` (modified) — added `vitest` as a dev dependency, `test`/`test:watch` scripts
